    ...
    <div class='denglu_wrapper'>
      ...
      <form class='dengru'>
        <p  v-for='(value,key) in initshuju'>
            <input class='shuru'
                   :type='(key.indexOf("name") != -1) ? "text" : "password"'
                   :class='key'
                   :name='key'
                   :placeholder='value'
                   :title='value'
  ...
  methods: {
    denglu: function () {
      let instance = axios.create({
        baseURL: 'http://10.0.0.106:8181',
        timeout: 1000
      })
      let data = Qs.stringify(this.tijiaoshuju)
      instance.post('/yonghu/denglu',
        data
      ).then(function (resp) {
        console.log('成功---')
    ...
    pattern: function () {
      ...
      let psdpattern = /^(?![0-9]+$)(?![a-zA-Z]+$)[0-9A-Za-z]{6,21}$/
      for (let i in this.tijiaoshuju) {
        if (i && i === 'username') {
          if (namepattern.test(this.tijiaoshuju[i])) {
            this.tishi[i] = '用户名正确'
            tishi[this.USERINDEX].style.color = '#0F7908'
          } else {
            this.tishi[i] = '用户名错误'
          ...
          if (psdpattern.test(this.tijiaoshuju[i])) {
            this.tishi[i] = '密码正确'
            tishi[this.PWDINDEX].style.color = '#0F7908'
          } else {
            this.tishi[i] = '密码有误'
          ...
          if (psdpattern.test(this.tijiaoshuju[i])) {
            ...
            this.tishi[i] = '6-16位，字母和数字，区分大小写'
            tishi[this.PWDINDEX].style.color = '#A2020F'
          }
